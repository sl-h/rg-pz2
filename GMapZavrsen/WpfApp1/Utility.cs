...
namespace WpfApp1
{
    public static class Utility
    {
        ...
        public static MyPoint GetBottomLeftPoint(List<MyPoint> source)
        {
            source.ForEach(s => ToLatLon(s.x, s.y, 34, out s.x, out s.y));
            return source.OrderBy(s => s.x).ThenBy(s => s.y).ToList().First();
        }

        public static double ConvertToCanvas(double point, double scale, double start, double size, double width)
        {
            var a = Math.Round((point - start) * scale / size) * size / width;
            return a;
        }
    }
}
